...
use crate::session::{SessionData, get_sessions, mark_session_for_closure_uuid, user_connected};
...
pub async fn serve() {
    ...
    let changes = warp::get()
        ...
        .and_then(|resource_path: String, uuid, lang| async move {
            match crate::compare::changes(
                decode(&resource_path)
                    .expect("Invlude URL encoding in changes")
                    .into_owned(),
                uuid,
                lang,
            )
